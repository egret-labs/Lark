 ...
 */
class DefaultAssetAdapterExample extends swan.Group {
    constructor() {
        super();
    ...
    private onAddedToStage(e: lark.Event) {
        ...
        this.stage.registerImplementation("swan.IAssetAdapter", assetAdapter);

        //在程序的其他地方，需要加载资源时可以使用下面的方法
        var adapter: swan.IAssetAdapter = this.stage.getImplementation("swan.IAssetAdapter");
        adapter.getAsset('resources/cloudy.png', this.assetLoaded, this);
    }

    private assetLoaded(content: any, source: string) {
        //由于我们请求的是一个图片 URL 所以这里拿到的 content 是一个 BitmapData
        //注意，这里只是为了演示 AssetAdapter 的用法，在实际使用中 swan.Image 的构造函数可以直接指定一个 URL，Image 会自动处理加载过程
        var image = new swan.Image(content);
        this.addChild(image);
    }
}